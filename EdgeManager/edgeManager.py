...
def lxc_give_access(client, ports, app):
    """
    Give access of LXC to the Cloud, so it can deploy Edge server
    """
...
def handshake(client):
    ...
    if has_enough_resource(unitCPU, unitMemory):
...
def handle(client,addr):
    ...
    if data == "hello":
        ...
        index = next((index for (index, d) in enumerate(lxcList) if d['App'] == request['App']), None)
